...
/**
 * 项目名称 : 智慧城市智慧养老
 * 文件名称 ： RoundedCornersCrop.java
 * 功能描述 ： glide圆角变换
 * 作   者 ： kangshuqian757
 * 日   期 ： 2019/7/30
 */
public class RoundedCornersCrop extends BitmapTransformation {
    ...
    public RoundedCornersCrop(int radius, int margin) {
        this(radius, margin, CornerType.ALL);
    }

    public RoundedCornersCrop(int radius, int margin, CornerType cornerType) {
        this.radius = radius;
    ...
    protected Bitmap transform(BitmapPool pool, Bitmap toTransform, int outWidth, int outHeight) {
        ...
        paint.setShader(new BitmapShader(toTransform, Shader.TileMode.CLAMP, Shader.TileMode
                .CLAMP));
    ...
    private void drawRoundRect(Canvas canvas, Paint paint, float width, float height) {
        ...
        switch (cornerType) {
            case ALL:
                canvas.drawRoundRect(new RectF(margin, margin, right, bottom), radius, radius,
                        paint);
                break;
            case TOP_LEFT:
    ...
    private void drawBottomRoundRect(Canvas canvas, Paint paint, float right, float bottom) {
        canvas.drawRoundRect(new RectF(margin, bottom - diameter, right, bottom), radius*10, radius*2,
                paint);
    ...
    private void drawOtherBottomLeftRoundRect(Canvas canvas, Paint paint, float right, float
            bottom) {
        canvas.drawRoundRect(new RectF(margin, margin, right, margin + diameter), radius, radius,
                paint);
        canvas.drawRoundRect(new RectF(right - diameter, margin, right, bottom), radius, radius,
    ...
    public String toString() {
        return "RoundedCornersCrop(radius=" + radius + ", margin=" + margin + ", diameter="
                + diameter + ", cornerType=" + cornerType.name() + ")";
    }

    @Override
    public boolean equals(Object o) {
        return o instanceof RoundedCornersCrop &&
                ((RoundedCornersCrop) o).radius == radius &&
                ((RoundedCornersCrop) o).diameter == diameter &&
                ((RoundedCornersCrop) o).margin == margin &&
                ((RoundedCornersCrop) o).cornerType == cornerType;
    }

    @Override
    public int hashCode() {
        return ID.hashCode() + radius * 10000 + diameter * 1000 + margin * 100 + cornerType
                .ordinal() * 10;
    }
